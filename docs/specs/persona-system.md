...
- **Sub-section IMMUTABILE markers in sync** (raised 2026-05-01 from lead-user reference §15 pattern). The lead-user reference instance now has `### Principio` *inside* `## §15 — INSIDER ROLE BOUNDARIES` carrying an inline `<!-- IMMUTABILE -->` marker, while §15's top-level marker stays `<!-- OPEN -->`. The proposer + applier already handle this (their HTML-marker scan reads the actual file at proposal time and respects nested markers), but the **sync template generator** only checks markers on the first 3 non-empty lines after each H2 heading and does not propagate sub-section markers into the framework template. A Phase 7 enhancement should: (a) extend `split_into_sections` to track H3 sub-headings within each H2 section; (b) detect `<!-- IMMUTABILE -->` on an H3 heading; (c) emit the framework template with the nested marker preserved; (d) treat such sub-sections as IMMUTABILE for sync-direction purposes (verbatim from doctrine source) while the parent H2 stays operator-authored. Test by syncing a synthetic source with a nested sub-section marker and asserting the framework template carries it.
