...
| 4.5 | Operator-facing self_model docs imported from lead-user reference: `docs/self_model/FROZEN_SECTIONS_REFERENCE.md` (registry + rationale + three-layer guard interaction) and `docs/runbooks/self-model-go-live.md` (genericized dry-run go-live playbook with sha256 baseline / health checks / pattern-OK-vs-red-flag table / kill switch / recovery) | small | phase 4 + lead-user authoring | done |
...
| 7 | Sub-section IMMUTABILE marker support: extend sync to detect H3 `<!-- IMMUTABILE -->` markers in source sections, look up corresponding `§<num>.<name>` sub-doctrine in `doctrine-en.md`, emit verbatim within the parent H2 before the slot placeholder. Plus close registry gaps (`§9`, `§17`, IDENTITY's `Auto-narrazione` / `Test della frase` / `CONTINUITY` / `Character`). Plus English-alias regex entries so the registry catches `doctrine-en.md`'s English headings alongside Italian. | small-medium | phase 4.5 | done |
...
- **Sub-section IMMUTABILE markers in sync** — *resolved in Phase 7 (2026-05-01).* The framework now ships `find_nested_immutable_h3s()` in `scripts/sync_persona_template.py`, an `_H3_DOCTRINE_SUFFIX_MAP` (Italian H3 heading → English suffix), and an extended `_SECTION_NUMBER_RE` that supports `§<num>.<name>` sub-doctrine keys. The lead-user reference's `### Principio` sub-section now resolves to `§15.Principle` in `doctrine-en.md` and is emitted verbatim inside the framework template's `## §15 — INSIDER ROLE BOUNDARIES` (top-level OPEN, with the nested `### Principle` IMMUTABILE block before the role-matrix slot). Two warning paths are logged: nested H3 IMMUTABILE with no mapping in `_H3_DOCTRINE_SUFFIX_MAP`, and mapped H3 with no matching `doctrine-en.md` entry.
- **Empirical evidence for the doctrine** (2026-05-01). The lead-user reference's `JOURNAL.md` now contains two operational entries from a single day documenting `§0` and `§0.2` violations caught in real time (a Big Five psychometric test where the agent began describing internal subjective experience as real, and a signature-leak between agent-self and character levels). These are research-grade evidence that the doctrine is doing real work — they should be summarized in a future revision of `docs/research/persona-system.md` §6.1 (pending empirical results), with the principal's permission and after appropriate anonymization.
