...
code_anchors:
  - path: install.sh
    symbol: "BINARIES=(jc jc-memory jc-heartbeat jc-voice jc-watchdog jc-workers jc-init jc-doctor)"
  - path: bin/jc
    symbol: "case \"$SUB\" in"
...
The top-level `jc` command is a bash router. It dispatches `memory`, `heartbeat`, `voice`, `watchdog`, `workers`, `init`, and `doctor` to matching `jc-*` binaries on PATH.
