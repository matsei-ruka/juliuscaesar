...
- **The enactment token must appear explicitly.** Default is `OK enact` (case-insensitive, trimmed). Casual agreement — "sure", "go ahead", "looks good" — does **not** enact. The token is a guard against ambiguous chat enacting structural changes by accident, not a secret.
