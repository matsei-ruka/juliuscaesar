...
def load_proposals(instance_dir: Path, state: str = "staging") -> Iterator[Proposal]:
    """Load proposals from storage. State: staging | applied | rejected."""
    path = instance_dir / "memory" / "staging" / f"user-model-{state}.jsonl"
    if not path.exists():
        return
...
def has_recent_proposal(
    ...
    path = instance_dir / "memory" / "staging" / "user-model-staging.jsonl"
    if not path.exists():
        return False
...
def move_proposal(
    ...
    from_path = instance_dir / "memory" / "staging" / f"user-model-{from_state}.jsonl"
    to_path = instance_dir / "memory" / "staging" / f"user-model-{to_state}.jsonl"
